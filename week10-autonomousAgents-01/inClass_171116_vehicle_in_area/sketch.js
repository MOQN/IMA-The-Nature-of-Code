...
class Vehicle {
  ...
  seek(target) {
    ...
    if (distance > this.brakeRad) {
      ...
      //slow down
      var mappedMag = map(distance, 0, this.brakeRad, 0, this.maxDesiredVel);
  ...
  checkArea() {
    ...
    if (this.pos.x < MARGIN) {
      var desiredVel = createVector(this.maxDesiredVel, this.vel.y);
      var steerForce = p5.Vector.sub(desiredVel, this.vel);
      steerForce.limit(this.maxSteerForce); // maxForce
      this.applyForce(steerForce);
    ...
    if (this.pos.y < MARGIN) {
      var desiredVel = createVector(this.vel.x, this.maxDesiredVel);
      var steerForce = p5.Vector.sub(desiredVel, this.vel);
      steerForce.limit(this.maxSteerForce); // maxForce
      this.applyForce(steerForce);
